...
def _crawl_naver_keywords(text):
    ...
    if text.find('music') != -1:
        ...
        for chart_text in soup.find_all("p", class_="title"):
            title.append(chart_text.get_text().strip('\n'))
        for singer_text in soup.find_all("p", class_="artist"):
            singer.append(singer_text.get_text().strip('\n'))
        for i in range(10):
            keywords.append(str(i+1) + "위 : " + title[i] + " / " + singer[i])
